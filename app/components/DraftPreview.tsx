...
import { RecipientSelector, type LoopsList } from "./RecipientSelector";
...
type DraftPreviewProps = {
  ...
  sendSuccess: boolean;
  /** Mailing lists fetched from Loops */
  loopsLists: LoopsList[];
  listsLoading: boolean;
  selectedListIds: string[];
  onToggleList: (id: string) => void;
  onSelectAllLists: () => void;
  onDeselectAllLists: () => void;
...
export function DraftPreview({
  ...
  const selectedListNames = loopsLists
    .filter((l) => selectedListIds.includes(l.id))
    .map((l) => l.name);

  const shipLabel =
    selectedListNames.length > 0
      ? selectedListNames.length <= 2
        ? selectedListNames.join(" & ")
        : `${selectedListNames.length} lists`
      : "No lists selected";

  const canShip = loopsConfigured && preflightOk && selectedListIds.length > 0;
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <button
                ...
                className={`inline-flex cursor-pointer items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium transition ${previewMode
            ...
            <div className="mt-3 flex flex-col gap-3">
              ...
              <div className="border-t border-white/40 pt-3">
                <RecipientSelector
                  ...
                  onDeselectAll={onDeselectAllLists}
                ...
                <AnimatePresence mode="wait">
                  {directShipSuccess ? (
                    ...
                    >
                      ...
                      <p className="text-lg font-semibold text-green-700">Newsletter Sent to {shipLabel}</p>
                    ...
                    >
                      <p className="mb-2 text-xs font-medium uppercase tracking-wider text-slate-500">
                        Direct Ship{selectedListNames.length > 0 ? ` to ${shipLabel}` : ""}
                      </p>
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          ...
                          className={`inline-flex cursor-pointer items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-50 ${confirmShip
